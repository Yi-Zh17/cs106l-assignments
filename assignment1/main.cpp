...
void parse_csv(std::string filename, std::vector<Course>& courses) {
  ...
  if (ifs.is_open()) {
    ...
    while(std::getline(ifs, line)) {
        ...
        Course record{splited[0], splited[1], splited[2]};
        courses.push_back(record);
    }
  }
  ifs.close();
}
...
void write_courses_offered(std::vector<Course>& all_courses) {
	...
	// Use a vector to keep track of courses to be deleted
	...
	if(ofs.is_open()) {
		...
		for(int i = 0; i < all_courses.size(); i++) {
			if(all_courses[i].quarter != "null") {
				...
				courses_offered.push_back(all_courses[i]);
			}
		}
	}
	ofs.close();
	// Delete courses offered
	for(int i = 0; i < courses_offered.size(); i++) {
		delete_elem_from_vector(all_courses, courses_offered[i]);
	}
}
